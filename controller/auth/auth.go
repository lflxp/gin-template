...
func Auth(c *gin.Context) {
	...
	authService := service.User{Username: reqInfo.Username, Password: reqInfo.Password}
...
func GetUserInfo(c *gin.Context) {
	...
	for _, v := range user.Role {
		RoleService := service.Role{ID: v.ID}
		...
		menus = utils.RemoveRepByMap(menus)
...
func DeleteUser(c *gin.Context) {
	...
	userService := service.User{ID: id}
	exists, err := userService.ExistByID()
	if err != nil {
		appG.Response(http.StatusInternalServerError, e.ERROR_EXIST_FAIL, nil)
		return
